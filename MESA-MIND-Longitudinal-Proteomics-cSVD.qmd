...
This section conducts a cross-sectional PWAS, using the set of covariates chosen by Tim & Lekki, and provides background to the longitudinal analysis (main analysis of interest) only.
...
FA_E1_LASSO_res_reducedcov <- targets::tar_read(FA_E1_LASSO_reducedcov)
FA_E1_LASSO_importantvars_reducedcov <- as.data.frame(FA_E1_LASSO_res_reducedcov$LASSO_output_important_vars)

#Protein_names

protein_names <- PWAS_res_reducedcov |>
  dplyr::pull(OlinkID)
...
-   The associations between PVS and proteins were analyzed via linear regression
-   For PVS the numeric covariates included `{r} print(epvs_numeric_covs_reducedcov)`. ^2^
-   For PVS the factor covariates included `{r} print(epvs_factor_covs_reducedcov)`. ^2^
-   PVS was transformed using an inverse normal transformation (with blom constant).

## PVS: Exam 6 proteins

-   The E6 proteins `->` E6 MRI for PVS included data from N=`{r} print(max(PWAS_res_reducedcov$epvs_6_nobs, na.rm=TRUE))` participants.
...
-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(epvs_6_fdr < .05 )|> nrow()` exam 1 proteins were significantly associated with PVS at an FDR corrected `P<.05`

### PVS: Exam 1 proteins

-   The E1 proteins `->` E6 MRI for PVS included data from N=`{r} print(max(PWAS_res_reducedcov$epvs_1_nobs, na.rm=TRUE))` participants.
...
-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(epvs_1_fdr < .05 )|> nrow()` exam 1 proteins were significantly associated with PVS at an FDR corrected `P<.05`.
...
-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(fa_1_fdr < .05 )|> nrow()` exam 1 proteins were significantly associated with FA at an FDR corrected `P<.05` (see @fig-fa-E1-reducedcov in Appendix 1).

-   Due to the large number of predictors, a penalized model was run (elastic net penalty) on a 50:50 test:training split of the data. The full results can be found [here](https://github.com/LekkiWood/MESA-MIND-Longitudinal-Proteomics-cSVD/blob/main/outputs/FA_E1_PWAS_reducedcov_fullLASSO.csv).

-   From the N=`{r} PWAS_res_reducedcov |> dplyr::filter(fa_1_fdr < .05 )|> nrow()` exam 1 proteins significantly associated with FA at an FDR corrected `P<.05`, the LASSO identified N=`r print(FA_E1_LASSO_importantvars_reducedcov |> dplyr::filter(Feature %in% protein_names) |> nrow())` proteins as variables of importance. These are described / visualized in @tbl-fa-E1-LASSO-reducedcov and @fig-fa-E1-LASSO-reducedcov below.

::: landscape
```{r}

#| label: tbl-fa-E1-LASSO-reducedcov
...
gt_table <-  PWAS_res_reducedcov |> 
  ...
  dplyr::filter(OlinkID %in% FA_E1_LASSO_importantvars_reducedcov$Feature) |>
...
```{r}
#| label: fig-fa-E1-LASSO-reducedcov
...
full_res <- PWAS_res_reducedcov |>
  dplyr::mutate(diffexpressed = dplyr::case_when(fa_1_beta > 0 & fa_1_fdr < 0.05 ~ "SIG INCREASE",
                ...
                colour = dplyr::case_when(fa_1_beta > 0 & fa_1_fdr < 0.05 ~ "purple", 
                                          ...
                                          TRUE ~ "slategrey"),
                label= dplyr::case_when(fa_1_fdr < 0.05 & OlinkID %in% FA_E1_LASSO_importantvars_reducedcov$Feature ~ Assay,
                                        TRUE ~ NA_character_))
...
#| tbl-cap: "Associations Between Exam 6 Proteins and Microbleeds Reaching an FDR-corrected P<.05 Equivalent"
#| cache: TRUE
...
-   The covariate choice was decided between Tim and Lekki, and mirrors that of the cross-sectional results.
...
-   A total of N=`{r} PWAS_res_long |> dplyr::filter(P_time_wmh_int_fdr < .05 )|> nrow()` proteins were significantly associated with WMH at an FDR corrected `P<.05` (see @fig-WMH-long in Appendix 2).
...
\newpage
:::landscape
# Appendix 1 : Figures displaying full results for assocations between MRI and single time point proteins, where there are many associations

## Fractional anisotropy and exam 1 proteins
...
# Appendix 2: Figures for Full Longitudinal Association Results

## White Matter Hyperintensities

```{r}
#| label: fig-WMH-long
#| fig-cap: "Associations Between Protein Trajectories and Subsequent White Matter Hyperintensities"
#| fig-width: 13
#| fig-height: 9
#| cache: TRUE
...
ggplot2::ggplot(data=full_res, ggplot2::aes(x=beta_int_time_wmh, y=-log10(P_time_wmh_int_fdr), 
