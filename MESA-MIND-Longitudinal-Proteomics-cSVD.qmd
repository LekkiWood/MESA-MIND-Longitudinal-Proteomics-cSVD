...
# Step 6: Cross sectional Protein-Wide Association Studies (PWAS) with a reduced number of covariates

This section repeats the cross-sectional PWAS, nusing thr set of covariates recommended by Tim.
...
-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(fa_6_fdr < .05 )|> nrow()` exam 6 proteins were significantly associated with FA at an FDR corrected P<.05. 

### FA: Exam 1 proteins

-   The E1 proteins -> E6 MRI for FA included data from N=`{r} print(max(PWAS_res_reducedcov$fa_1_nobs, na.rm=TRUE))` participants.

-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(fa_1_fdr < .05 )|> nrow()` exam 1 proteins were significantly associated with FA at an FDR corrected P<.05.
...
-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(mb_present_6_fdr < .05 )|> nrow()` exam 6 proteins were significantly associated with MB at an FDR corrected P<.05.
...
-   A total of N=`{r} PWAS_res_reducedcov |> dplyr::filter(mb_present_1_fdr < .05 )|> nrow()` exam 1 proteins were significantly associated with MB at an FDR correct P<.05.
