cross_sectional_logistic_PWAS_function <- function(cleaned_proteins, protein_mapping,
  ...
  names(out) <- paste(paste(paste(outcome, "E", sep="_"), chosen_exam, sep=""), names(out), sep="_")
  
  results <- as.data.frame(out) |>
    dplyr::rename(OlinkID = paste(paste(paste(outcome, "E", sep="_"), chosen_exam, sep=""), "protein", sep="_")) |>
